...
function renderAthleteList(athletes) {
  ...
  box.innerHTML = athletes.map((a) => `<div class="athlete-card ${a.id === selectedAthleteId ? 'active' : ''}" onclick="selectAthlete('${a.id}')"><h3>${esc(a.name)}</h3><div class="small">Class ${esc(a.gradYear || '—')} • ${esc(a.primaryPosition || 'Position TBD')} • ${esc(a.team || 'No team')}</div></div>`).join('');
}
